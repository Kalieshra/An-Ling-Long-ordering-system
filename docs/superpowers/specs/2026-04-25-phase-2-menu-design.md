...
Two migrations land in this phase, in this order (the extension migration MUST run first so the GIN trigram index in the auto-generated initial succeeds):

1. **`menu/migrations/0001_pg_trgm.py`** — hand-written `RunSQL("CREATE EXTENSION IF NOT EXISTS pg_trgm", reverse_sql="DROP EXTENSION IF EXISTS pg_trgm")`. Empty `dependencies = []`. Applies before any model migration.
2. **`menu/migrations/0002_initial.py`** — auto-generated by `makemigrations menu` after step 1 lands. Creates Category, MenuItem (with the `GinIndex(opclasses=["gin_trgm_ops"])` declared in Meta.indexes), ModifierGroup, ModifierOption, Ingredient, Recipe. `dependencies = [("menu", "0001_pg_trgm"), ("__first__")]`.

Workflow during implementation: write 0001 by hand first → run `makemigrations menu` → it generates 0002 with the right dependency on 0001. The GIN index is part of `Meta.indexes` from the start so `makemigrations --check` never drifts.

### 4.4 New runtime dependencies

Two packages are added to `pyproject.toml [project] dependencies`:

- `django-filter>=24.0` — the `MenuItemFilter` and DRF integration
- `Pillow>=10.4` is already present (added in Phase 1 for `ImageField` support)

`uv sync` regenerates `uv.lock`.

### 4.5 Shared services (none)
